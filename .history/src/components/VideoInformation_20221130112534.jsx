...
function VideoInformation({ videoInfo }) {
  ...
  // const { title, channelTitle, description, channelId } = videoInfo.snippet;
  ...
  useEffect(() => {
    fetch(
      `https://www.googleapis.com/youtube/v3/channels?id=${videoInfo.snippet.channelId}&part=snippet,statistics&key=${process.env.REACT_APP_KEY}`
    ) //
      .then((res) => res.json()) //
